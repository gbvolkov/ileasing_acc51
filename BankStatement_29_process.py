...
def BankStatement_29_process(header: pd.DataFrame, data: pd.DataFrame, footer: pd.DataFrame, inname: str, clientid: str, sheet: str, logf: TextIOWrapper) -> pd.DataFrame:
    ...
    df["cpAcc"] = data["реквизитыплательщика/получателяденежныхсредств.номерсчета"]
    ...
    df["Debet"] = data['суммаоперациипосчету.подебету']
    df["Credit"] = data['суммаоперациипосчету.покредиту']
