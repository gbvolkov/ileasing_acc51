...
def BankStatement_29_process(header: pd.DataFrame, data: pd.DataFrame, footer: pd.DataFrame, inname: str, clientid: str, sheet: str, logf: TextIOWrapper) -> pd.DataFrame:
    ...
    df["cpAcc"] = data["Реквизиты плательщика/получателя денежных средств.номер счета "]
    ...
    df["Debet"] = data['Сумма операции по счету .по дебету']
    df["Credit"] = data['Сумма операции по счету .по кредиту']
