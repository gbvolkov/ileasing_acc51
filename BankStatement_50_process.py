...
def set_footer_fields(footer, df):
    cbalance = footer[footer.iloc[:, 0].str.startswith("Исходящий остаток")].dropna(
            axis=1, how="all"
        )
    if cbalance.size >= 1:
        df["closingBalance"] = cbalance.iloc[:, 1].values[0]
    turnovers = footer[footer.iloc[:, 0].str.startswith("Итого")].dropna(
            axis=1, how="all"
        )
    if turnovers.size > 1:
