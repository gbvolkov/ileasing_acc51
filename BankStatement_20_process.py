...
#           "filename"]
def BankStatement_20_process(header: pd.DataFrame, data: pd.DataFrame, footer: pd.DataFrame, inname: str, clientid: str, params: dict, sheet: str, logf: TextIOWrapper) -> pd.DataFrame:
    df = pd.DataFrame(columns = COLUMNS)

    df["entryDate"] = data["датаоперации"]

    df["cpBIC"] = data.apply(lambda row: row['бикswiftбанкаплат'] if pd.isna(row['дебет']) else row['бикswiftбанкаполуч'], axis=1)
    ...
    df["cpAcc"] = data.apply(lambda row: row['nсчетаплательщика'] if pd.isna(row['дебет']) else row['nсчетаполучателя'], axis=1)
    ...
    df["clientBIC"] = data.apply(lambda row: row['бикswiftбанкаплат'] if pd.isna(row['кредит']) else row['бикswiftбанкаполуч'], axis=1)
    ...
    df["clientAcc"] = data.apply(lambda row: row['nсчетаплательщика'] if pd.isna(row['кредит']) else row['nсчетаполучателя'], axis=1)
