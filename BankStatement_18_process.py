...
def BankStatement_18_process(header: pd.DataFrame, data: pd.DataFrame, footer: pd.DataFrame, inname: str, clientid: str, sheet: str, logf: TextIOWrapper) -> pd.DataFrame:
    ...
    df["cpAcc"] = data["Реквизиты плательщика/получателя денежных средств.номерсчета(специальногобанковскогосчета)"]
    ...
    df["Debet"] = data['Сумма операции по счету (специальному банковскому счету).подебету']
