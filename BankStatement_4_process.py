...
#№док|датадокумента|датаоперации|реквизитыкорреспондента.наименование|реквизитыкорреспондента.счет|реквизитыкорреспондента.иннконтрагента|реквизитыкорреспондента.банк|дебетсумма/суммавнп|кредитсумма/суммавнп|курсцбнадатуоперации|основаниеоперации(назначениеплатежа)
...
def BankStatement_4_process(header: pd.DataFrame, data: pd.DataFrame, footer: pd.DataFrame, inname: str, clientid: str, sheet: str, logf: TextIOWrapper) -> pd.DataFrame:
    ...
    df["cpBIC"] = data["реквизитыкорреспондента.банк"]
    #df["cpBank"] = data["Контрагент.Наименование банка "]
    df["cpAcc"] = data["реквизитыкорреспондента.счет"]
    df["cpTaxCode"] = data["реквизитыкорреспондента.иннконтрагента"]
    df["cpName"] = data["реквизитыкорреспондента.наименование"]
    df["Debet"] = data["дебетсумма/суммавнп"]
    df["Credit"] = data["кредитсумма/суммавнп"]
    df["Comment"] = data["основаниеоперации(назначениеплатежа)"]
