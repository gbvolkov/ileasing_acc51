...
def set_footer_fields(footer, df):
    cbalance = footer[
            footer.iloc[:, 0]
            == "Исходящий остаток на конец дня (периода) / Closing balance"
        ].dropna(axis=1, how="all")
    if cbalance.size > 1:
        df["closingBalance"] = cbalance.iloc[:, 1].values[0]
    turnovers = footer[footer.iloc[:, 0] == "Обороты / Turnover"].dropna(
            axis=1, how="all"
        )
    if turnovers.size > 1:
...
def set_header_fields(header, df):
    acc = header[header.iloc[:, 0] == "Номер счета / Account number:"].dropna(
            axis=1, how="all"
        )
    if acc.size > 1:
        df["clientAcc"] = acc.iloc[:, 1].values[0]
    client = header[header.iloc[:, 0] == "Клиент / Client:"].dropna(
            axis=1, how="all"
        )
    if client.size > 1:
    ...
    obalance = header[
            header.iloc[:, 0]
            == "Входящий остаток на начало дня (периода) / Opening Balance:"
        ].dropna(axis=1, how="all")
    if obalance.size > 1:
