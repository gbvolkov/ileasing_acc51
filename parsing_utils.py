...
def getFilesList(log: str, start: int, end: int, doctype: str = "выписка") -> list[str]:
    ...
    filelist = df['filename'][(df['status']=='PROCESSED') & (df['doctype'] == doctype)]
...
def process_data_from_preanalysis(process_func, preanalysislog, logname, outbasename, bSplit, maxFiles, doneFolder, FILEEXT, start, end):
    cnt = 0
        ...
        for fname in filter(lambda file: any(ext for ext in FILEEXT if (file.lower().endswith(ext))), fileslist):
            if Path(fname).is_file():
                ...
                try:
                    ...
                    try :
                        cnt += runParsing(process_func, clientid, outname, inname, doneFolder, logf)
